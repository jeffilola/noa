...
- Active M7 follow-up: PR #99, `M7 closeout: restore holder compliance records page`.
...
Results will be updated after the validation rerun on this branch.
...
1. M7 org admin view: run `pnpm qa:prepare`, sign in as `DEMO_CLERK_USER_ID`, open `/org/users`, click **Access view**, and confirm **Site safety orientation** plus **Electrical safety certification** appear in the access decision panel.
2. M7 holder view: switch to **Identity Holder**, open **Training & certs** or `/user/compliance`, and confirm the same training/certification rows render.
3. M7 refresh path: click **Refresh list** on `/user/compliance` and confirm the table reloads without an API error.
4. M7 visual check: toggle dark mode and confirm both org access panel and holder compliance table remain readable.
